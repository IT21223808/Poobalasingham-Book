...
export default function AccountsReceivablePage() {
  const [records, setRecords] = useState<AccountsReceivableItem[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<AccountsReceivableItem | null>(null);
  ...
  const [historyRecord, setHistoryRecord] = useState<AccountsReceivableItem | null>(null);
  ...
  const openPayModal = (record: AccountsReceivableItem) => {
  ...
  const openHistoryModal = (record: AccountsReceivableItem) => {
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    if (amount > selectedRecord.outstanding + 0.01)
      return setError(
        `Payment cannot exceed outstanding amount of Rs. ${selectedRecord.outstanding.toLocaleString("en-LK", { minimumFractionDigits: 2 })}`
      );
    ...
    try {
      ...
      await financeService.createCustomerPayment({
        customerId: selectedRecord.customerId as number,
        salesInvoiceId: selectedRecord.invoiceNumber,
      ...
      setSuccessMsg(
        `Payment of Rs. ${amount.toLocaleString("en-LK", { minimumFractionDigits: 2 })} received successfully!`
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Accounts Receivable (AR)</h2>
          <p className="text-xs text-gray-500">
            Track customer outstanding balances from POS sales and credit invoices
          </p>
        </div>
        <button
          onClick={fetchAR}
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <h3 className="mt-1.5 text-lg font-bold text-gray-900">{formatCurrency(summary.totalReceivable)}</h3>
          <p className="text-[11px] text-gray-400">{summary.invoiceCount} invoices</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <span className="text-xs font-medium uppercase tracking-wider text-gray-500">Total Received</span>
          <h3 className="mt-1.5 text-lg font-bold text-emerald-600">{formatCurrency(summary.totalReceived)}</h3>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <p className="text-[11px] text-red-400">{summary.overdueCount} overdue</p>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 bg-white p-3.5 rounded-xl border border-gray-200">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && fetchAR()}
        ...
        <button onClick={fetchAR} className="rounded-lg bg-blue-600 px-3.5 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">
          Apply
        </button>
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-gray-50 text-gray-500 uppercase tracking-wider font-semibold border-b border-gray-200">
              <tr>
                ...
                <th className="px-5 py-3.5 text-right text-emerald-700">Received</th>
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700">
              {records && records.length > 0 ? (
                records.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50/75 transition">
                    ...
                    <td className="px-5 py-3 text-center">{getStatusBadge(r.status)}</td>
                    <td className="px-5 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        {r.outstanding > 0 && r.customerId && (
                          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between pb-4 border-b border-gray-100">
              <h3 className="text-sm font-bold text-gray-900">Receive Customer Payment</h3>
            ...
            <div className="mt-4 rounded-lg bg-gray-50 border border-gray-200 p-4 text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-gray-900">{selectedRecord.customerName}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{formatCurrency(selectedRecord.invoiceAmount)}</span>
              </div>
              <div className="flex justify-between">
            ...
            <form onSubmit={handlePaySubmit} className="mt-4 space-y-3 text-xs">
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0.01" max={selectedRecord.outstanding}
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Invoice #{historyRecord.invoiceNumber} — {historyRecord.customerName}
                </p>
              </div>
            ...
            <div className="mt-4">
              {historyRecord.payments && historyRecord.payments.length > 0 ? (
                <div className="space-y-2">
                  {historyRecord.payments.map((p: CustomerPayment) => (
